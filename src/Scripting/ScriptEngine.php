...
class ScriptEngine
{
     ...
     */
    protected static $_passFullEvent = false;
    /**
     ...
     */
    public static function create( $libraryScriptPath = null, array $variables = array(), array $extensions = array(), $reportUncaughtExceptions = true, $passFullEvent = false )
    {
        //  Create the engine
        if ( !static::$_engine )
        {
            ...
            static::setPassFullEvent( $passFullEvent );

        }

        $_mirror = new \ReflectionClass( get_class( static::$_engine ) );
        ...
        {
            Log::notice( 'CommonJS require() support not available in your version of V8Js' );
        }
        else
        {
            static::$_moduleLoaderAvailable = true;
    ...
    protected static function _registerExtensions()
    {
        ...
        {
            /** @noinspection PhpUndefinedMethodInspection */
            static::registerExtension( $_module, str_replace( '{module}', $_module, static::MODULE_LOADER_TEMPLATE ), array(), false );
        }
    }
    ...
    public static function enrobeScript( $script, array $normalizedEvent )
    {
        ...
        $_eventData = json_encode( Option::get( $normalizedEvent, 'payload', array() ), JSON_UNESCAPED_SLASHES );
...
EOT;
        ...
        if ( static::$_passFullEvent )
        {
            $_declarationSignature = 'event, eventData';
            $_callSignature = '_event, _eventData';
        }
        else
        {
            $_declarationSignature = 'event, eventAll';
            $_callSignature = '_eventData, _event';
        }
...
var PHP = DSP;
var _event = {$_event};
var _eventData = {$_eventData};

function _processPlatformEvent( {$_declarationSignature} ) {
    {$script}
    ;
    return event;
}

_result = _processPlatformEvent({$_callSignature});
...
SCRIPT;
     ...
     * @return boolean
     */
    public static function getPassFullEvent()
    {
        return static::$_passFullEvent;
    }
     ...
     * @return ScriptEngine
     */
    public static function setPassFullEvent( $passFullEvent )
    {
        static::$_passFullEvent = $passFullEvent;
    }
