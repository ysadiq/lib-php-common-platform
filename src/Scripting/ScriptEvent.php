...
class ScriptEvent
{
    ...
    public static function normalizeEvent( $eventName, PlatformEvent $event, $dispatcher, array $extra = array(), $includeDspConfig = true, $returnJson = false )
    {
        ...
        if ( !$_config )
        {
            $_config = ( $includeDspConfig ? Platform::storeGet( Config::LAST_RESPONSE_CACHE_KEY, false ) : false );
        }
     ...
     * @return $this
     */
    public static function updateEventFromHandler( PlatformEvent &$event, array $exposedEvent = array() )
    {
    ...
    {
        $_data = ( $event ? $event->getRequestData() : $event );
